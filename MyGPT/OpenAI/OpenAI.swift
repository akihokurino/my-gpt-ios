        ...
        where T: OpenAIRequestProtocol, V: Codable, T.ResponseType == V
    {
        ...
        return Future { promise in
            let api = AF.request(request)
                ...
                .responseDecodable(of: V.self, decoder: decoder) { response in
                    switch response.result {
                    case let .success(result):
                        promise(.success(result))
                    case let .failure(error):
                        print("OpenAI API error: \(error.errorDescription)")
                        promise(.failure(AppError.plain(error.errorDescription ?? defaultErrorMessage)))
                    }
                }
            api.resume()
        }
